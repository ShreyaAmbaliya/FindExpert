...
{ 
	...
	function view_chat($id=0)
	{
		...
		$qry=$this->db->get_where('client',array('client_id'=>$res['client_id']));
		$rows=$qry->row_array();
		$data['client']=$rows;
			
		$this->load->view('worker/worker_chat',$data); 
	}
	function msg_send($id=0)
	{
		$msg=$this->input->post('text_msg');
				...
				redirect('worker/manage_chat/view_chat/'.$this->input->post('bid_id'));
			}else if($msg!=""){
		$saveArr=array(
			'text_msg'=>$msg,
			'assign_id'=>$id,
